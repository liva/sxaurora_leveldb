...
docker run --rm -it -v $PWD:$PWD -w $PWD leveldb-build-base:ve sh -c 'mkdir -p build && cd build && CC=/opt/nec/nosupport/llvm-ve/bin/clang CXX=/opt/nec/nosupport/llvm-ve/bin/clang++ AR=/opt/nec/nosupport/llvm-ve/bin/llvm-ar CFLAGS="--target=ve-linux" CXXFLAGS="--target=ve-linux" LDFLAGS="--target=ve-linux" cmake3 -DCMAKE_BUILD_TYPE=Release -DCMAKE_VERBOSE_MAKEFILE=ON .. && cmake3 --build . -j 12'
